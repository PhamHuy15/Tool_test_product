...
- `Viết Test Case`: Dựa vào cấu trúc JSON cấu hình của trang web để sinh bộ test case kiểm thử thủ công chi tiết (đóng vai trò Senior QA Tester 10 năm kinh nghiệm), xuất định dạng Markdown và tệp CSV phục vụ import trực tiếp lên Jira/Xray.
...
1. Mở web app tại `http://127.0.0.1:4545`.
2. Chọn tab thao tác `Viết Test Case`.
3. Dán JSON test package muốn tạo test case hoặc bấm nút `JSON Mẫu` để nạp dữ liệu mẫu nhanh. Bạn cũng có thể kéo/thả hoặc bấm `Tải File` để tải lên tệp cấu hình `.json` có sẵn.
4. Bấm `Tạo Test Case`.
5. Chờ hệ thống phân tích và viết test case (tiến trình chạy Codex được hiển thị live trong tab `Tiến trình`).
6. Khi hoàn tất, hệ thống tự động điều hướng sang tab `Test Cases` hiển thị bộ test cases hoàn chỉnh dưới dạng bảng chuyên nghiệp.
7. Xem/tải kết quả:
   - Bấm `Tải MANUAL_TEST_CASES.md` để tải file báo cáo định dạng Markdown.
   - Bấm `Tải manual_test_cases.csv` để lưu file test case dạng bảng hỗ trợ tiếng Việt (UTF-8) để import trực tiếp vào Jira/Xray.
...
- `POST /api/run-manual-test-cases`: Chạy sinh bộ test case kiểm thử thủ công từ JSON test package.
- `GET /api/runs/:runId/events`: Stream tiến trình bằng SSE.
- `GET /api/runs/:runId/files/:fileName`: Tải file kết quả (bao gồm TEST_REPORT.md, MANUAL_TEST_CASES.md, v.v.).
